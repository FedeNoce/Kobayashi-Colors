...
for i in range(170000):
    if i % 500 == 0:
        filename = str(i) + '.png'
        img_o = cv2.imread('/data/fnocentini/outfits/' + filename)
        img_a = img_o
        # convert to graky
        ...
        plt.imshow([clusters])
        plt.savefig('/data/fnocentini/tmp/cluster.png')
        #plt.show()
        ...
        if distance <= 10:
            img_o = np.reshape(img_o, (150, 300, 3)).astype('float')
            clusters = np.reshape(clusters, (3, 1, 3))
            # clusters[:, :, [0, 2]] = clusters[:, :, [2, 0]]
            # bt = np.reshape(best_triplet.get_rgb(), (3, 1, 3))
            # bt[:, :, [0, 2]] = bt[:, :, [2, 0]]
            bt = best_triplet.get_rgb()
            plt.imshow([bt])
            plt.savefig('/data/fnocentini/tmp/b.png')
            bt = cv2.imread('data/fnocentini/tmp/b.png')
            clusters = cv2.imread('data/fnocentini/tmp/cluster.png')
            im_v = cv2.hconcat([np.array(img_a).astype('float'), np.array(img_o)])
            clust = cv2.hconcat([clusters, bt])
            cv2.imwrite('data/fnocentini/tmp/imgs.png', im_v)
            cv2.imwrite('data/fnocentini/tmp/triplets.png', clust)
            imgs = cv2.imread('data/fnocentini/tmp/imgs.png')
            triplets = cv2.imread('data/fnocentini/tmp/triplets.png')
            resized = cv2.resize(triplets, (600, 150))
            total = cv2.vconcat([np.array(imgs).astype('float64'), np.array(resized).astype('float64')])
            cv2.imwrite('data/fnocentini/tmp/attemp.png', total)
            img = cv2.imread('data/fnocentini/tmp/attemp.png')
            cv2.putText(img, 'Original Image', (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0, 1), 1)
            cv2.putText(img, 'Kobayashi Space', (320, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0, 1), 1)
            cv2.putText(img, 'Triplet of Kobayashi Image', (20, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0, 1), 1)
            cv2.putText(img, 'Best Matching', (320, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0, 1), 1)
            cv2.putText(img, str(best_triplet.class_), (300, 290), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0, 1), 1)
            cv2.putText(img, str(best_triplet.subclass_), (400, 290), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0, 1), 1)
            cv2.imwrite('data/fnocentini/imgs/' + filename, img)
